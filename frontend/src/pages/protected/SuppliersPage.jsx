...
const SuppliersPage = () => {
  ...
  const loadSuppliers = useCallback(async (q = '') => {
    ...
    try {
      const res = q.trim()
        ? await suppliersAPI.search(q.trim())
        : await suppliersAPI.getAll();
      setSuppliers(res.data.suppliers || []);
    } catch (err) {
      toast(err.response?.data?.message || 'Failed to load suppliers', 'error');
    } finally {
      setLoading(false);
    }
  }, [toast]);
  ...
  return (
    <AppLayout title="Suppliers" subtitle={`${activeCount} active supplier${activeCount !== 1 ? 's' : ''}`}>
      ...
      {loading ? (
        ...
        <div className="empty-state" style={{ background: 'var(--slate-900)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14 }}>
            ...
            <div key={s._id} className="card animate-fadeInUp" style={{ animationDelay: `${i * 0.07}s` }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
                <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 700, color: 'white', fontSize: 15, fontFamily: 'var(--font-display)' }}>{s.name}</div>
                    <div style={{ fontSize: 12, color: 'var(--slate-500)' }}>{s.contactPerson}</div>
                  </div>
                </div>
                <span className={`badge ${s.isActive ? 'badge-green' : 'badge-slate'}`}>{s.isActive ? 'Active' : 'Inactive'}</span>
              </div>

              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
                {[
                  { icon: 'Mail', val: s.email },
                  { icon: 'Activity', val: s.phone },
                  { icon: 'Globe', val: s.address },
                ].map((r, ri) => (
                  <div key={ri} style={{ display: 'flex', gap: 8, alignItems: 'center', fontSize: 12, color: 'var(--slate-400)' }}>
                    <Icon name={r.icon} size={12} style={{ color: 'var(--slate-600)', flexShrink: 0 }} />
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 6, padding: '6px 10px', marginBottom: 12 }}>
                <div style={{ fontSize: 10, color: 'var(--slate-600)', marginBottom: 2 }}>Supplier ID (use in Products)</div>
                <code style={{ fontSize: 11, color: 'var(--orange-400)', wordBreak: 'break-all' }}>{s._id}</code>
              </div>

              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 4, paddingTop: 12, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
